  ...
  //recognition.start();
    ...
    recognition.onresult = function(event) {
      ...
      //alert($scope.recognizedText);
        ...
        if($scope.data.chosenOption == 'session') {
            alert("before session within recognition");
          $scope.session($scope); 
        }else{
          alert("before choose option");
          $scope.chooseOption($scope);
          alert("after choose option");
        }
...
$scope.SwitchFuction($scope);
}


$scope.SwitchFuction = function ($scope) {
...
alert("before fetch");
  $http.get("http://lexico.pythonanywhere.com/vocabsession/api/getSession/riddhima/").then(function(response)
  { 
  $scope.details = response.data;
  alert("after fetch");
  alert($scope.details[1].word);
  $scope.length = response.data.length;
    alert("calling session function from within fetch");
  $scope.session($scope); 
